"""Tests for the shared comms-backup reader (scripts/comms_backup_loader.py).

Uses synthetic XML fixtures written to tmp_path, so it needs no real backup and
runs anywhere (CI, alice-vm, Bob's VM). This file is the "anchor with tests" Bob
asked for: it pins the shared reader's behavior so both comms_summary.py
(aggregates) and extract_erik_sms.py (sent-text corpus) can refactor against it.
"""
...
MODULE_PATH = (
    Path(__file__).resolve().parent.parent / "scripts" / "comms_backup_loader.py"
)
spec = importlib.util.spec_from_file_location("comms_backup_loader", MODULE_PATH)
...
def test_normalize_number_same_input_for_both_hashers(tmp_path: Path) -> None:
...
def test_epoch_ms_to_day_utc(tmp_path: Path) -> None:
...
def test_latest_file_respects_prefix(tmp_path: Path) -> None:
    ...
    assert cbl.latest_file(tmp_path, "calls-").name == "calls-20260603000000.xml"
