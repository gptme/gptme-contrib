...
NEXT_FOCUS: <one concrete specific action the next iteration should take, different from previous attempts>"
    ...
    if [[ "${cause}" == "infrastructure_bug" && "${bug_detail}" != "NONE" ]]; then
        ...
        gh issue create --repo gptme/gptme \
...
if [[ ${IMPROVED} -gt 0 ]]; then
    ...
    if [[ "${THRESHOLD_MET}" == "yes" ]]; then
        ...
        EXISTING_PR="$(gh pr list --repo gptme/gptme --head "${PUSH_BRANCH}" --json number --jq '.[0].number' 2>/dev/null || true)"
        if [[ -n "${EXISTING_PR}" ]]; then
            echo "PR already exists: gptme/gptme#${EXISTING_PR} — skipping PR creation"
        else
...
*Generated by autoresearch loop — do not merge until manually reviewed.*"
            PR_URL="$(gh pr create --repo gptme/gptme --base master --head "${PUSH_BRANCH}" \
        ...
        echo "To create PR manually: gh pr create --repo gptme/gptme --base master --head ${PUSH_BRANCH}"
    fi
fi
