...
try:
    ...
    # won't have it. The no-op fallback is safe here because the patterns
    # public_safe strips — workspace paths (/home/bob/bob/…), internal
    # hostnames (*.bjareho.lt), SSH remotes, and LXC container refs — are
    # specific to Bob's private deployment and will never appear in tweet drafts
    # generated by a standalone user. Degrade to a no-op with a warning so
    # the script remains usable without the full workspace.
    def public_safe(text: str) -> str:
        logging.getLogger(__name__).warning(
            "bobutils not installed — skipping public_safe sanitization"
            " (safe: Bob-specific private-ref patterns not present in standalone env)"
        )
        return text
