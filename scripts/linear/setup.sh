...
check_prerequisites() {
    ...
    if command -v python3 &> /dev/null; then
        ...
        # Use arithmetic comparison: Python 4+ OR Python 3.10+
...
prompt() {
    ...
    # Use printf -v for safer variable assignment (avoids eval)
...
main() {
    ...
    # Use umask to ensure file is created with 600 permissions (no timing gap)
    (umask 077 && cat > "$LINEAR_DIR/.env" << EOF
...
EOF
    ...
    # Add .env to .gitignore if not present (create .gitignore if needed)
    local GITIGNORE="$AGENT_WORKSPACE/.gitignore"
    if ! grep -q "scripts/linear/.env" "$GITIGNORE" 2>/dev/null; then
        touch "$GITIGNORE"  # Create if doesn't exist
...
EOF
    ...
    if [[ "$run_oauth" =~ ^[Yy] ]]; then
        # Use subshell to ensure proper working directory context
        (cd "$LINEAR_DIR" && uv run linear-activity.py auth) || {
            warn "OAuth flow failed. You can retry manually later with:"
            warn "  cd $LINEAR_DIR && uv run linear-activity.py auth"
        }
    fi
    ...
    if [[ "$start_services" =~ ^[Yy] ]]; then
        systemctl --user start "${AGENT_NAME}-ngrok.service" || {
            error "Failed to start ngrok service"
        }
        sleep 2
        ...
        echo ""
        info "Service status:"
        # Show status but don't fail the script if status display fails
        systemctl --user status "${AGENT_NAME}-ngrok.service" --no-pager 2>&1 || warn "Could not get ngrok service status"
        echo ""
        systemctl --user status "${AGENT_NAME}-linear-webhook.service" --no-pager 2>&1 || warn "Could not get webhook service status"
    fi
