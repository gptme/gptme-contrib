...
check)
    ...
    if _has_greptile_review; then
        ...
        if _needs_re_review; then
            ...
            # acked but never posted a review after ACK_GRACE_SECONDS), allow re-trigger even
            ...
            # "stale" (trigger was never acked) must NOT bypass: re-trigger only with new commits.
    ...
    if _has_greptile_review; then
        if _needs_re_review; then
            ...
            if [ "${_total_triggers:-0}" -ge "$MAX_TOTAL_TRIGGERS" ]; then
                ...
                # Plain "stale" (no ack) must still apply the guard to prevent spam.
